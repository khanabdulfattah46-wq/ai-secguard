...
export default function App() {
  ...
  const handleApiCall = async (endpoint, payload) => {
    ...
    if (activeTab === 'pwned') {
      ...
      if (!emailRegex.test(inputData)) {
        setResults({ error: "Invalid format! Please enter a correct email address (e.g. user@gmail.com)." });
        return;
      }
    } else if (activeTab === 'web' || activeTab === 'ai') {
      if (!inputData.startsWith('http://') && !inputData.startsWith('https://')) {
        setResults({ error: "Invalid format! Please enter a valid URL starting with http:// or https://" });
        return;
      }
    }
  ...
  const renderSidebar = () => (
    <div className="w-64 bg-gray-950 border-r border-gray-800 min-h-screen p-4 flex-col hidden md:flex">
  ...
  const renderMobileNav = () => (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-gray-950 border-t border-gray-800 flex justify-around items-center p-3 z-50">
      <button onClick={() => {setActiveTab('pwned'); resetState();}} className={`flex flex-col items-center gap-1 ${activeTab === 'pwned' ? 'text-blue-500' : 'text-gray-500'}`}>
        <Database className="w-5 h-5" />
        <span className="text-[10px] font-bold">Breach</span>
      </button>
      
      <button onClick={() => {setActiveTab('text'); resetState();}} className={`flex flex-col items-center gap-1 ${activeTab === 'text' ? 'text-blue-500' : 'text-gray-500'}`}>
        <Smartphone className="w-5 h-5" />
        <span className="text-[10px] font-bold">Phishing</span>
      </button>

      <button onClick={() => {setActiveTab('web'); resetState();}} className={`flex flex-col items-center gap-1 ${activeTab === 'web' ? 'text-blue-500' : 'text-gray-500'}`}>
        <div className="relative">
          <Globe className="w-5 h-5" />
          {!isAuthenticated && <Lock className="w-3 h-3 absolute -top-1 -right-2 text-gray-400" />}
        </div>
        <span className="text-[10px] font-bold">Web</span>
      </button>

      <button onClick={() => {setActiveTab('ai'); resetState();}} className={`flex flex-col items-center gap-1 ${activeTab === 'ai' ? 'text-blue-500' : 'text-gray-500'}`}>
        <div className="relative">
      ...
      {/* Login / Profile Mobile Button */}
      <button onClick={() => {setActiveTab('login'); resetState();}} className={`flex flex-col items-center gap-1 ${activeTab === 'login' ? 'text-emerald-500' : 'text-gray-500'}`}>
      ...
      <div className="bg-gray-900 border border-gray-800 p-8 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        {isAuthenticated ? (
          <div className="text-center">
            ...
            <p className="text-emerald-500 font-bold mb-8">Premium Account Active</p>
            <button onClick={handleLogout} className="w-full bg-red-600/20 text-red-500 hover:bg-red-600/30 border border-red-500/30 py-3 rounded-lg font-bold flex justify-center items-center gap-2 transition-all">
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 animate-in fade-in">
          ...
          <button onClick={() => setActiveTab('login')} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-lg font-bold shadow-lg transition-all flex items-center justify-center gap-2 mx-auto">
            <UserPlus className="w-5 h-5" /> Login to Access
          </button>
        </div>
      );
    }
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-8 border-b border-gray-800 pb-6 gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3"><Icon className="text-blue-500 w-7 h-7 md:w-8 md:h-8" /> {current.title}</h2>
            <p className="text-gray-400 mt-2 text-sm md:text-base">{current.desc}</p>
          </div>
          
          {activeTab !== 'pwned' && (
            <div className="flex items-center gap-3 bg-gray-900 border border-gray-800 px-4 py-2 rounded-lg w-fit">
        ...
        <div className="bg-gray-900 border border-gray-800 p-4 md:p-6 rounded-xl shadow-2xl">
          {current.isTextArea ? (
        ...
        {results && results.error && (
          <div className="mt-8 bg-red-950/20 border border-red-500/50 rounded-xl p-6 text-center animate-in fade-in">
            ...
            <h3 className="text-xl font-bold text-red-500 mb-2">Input Error</h3>
            <p className="text-gray-300">{results.error}</p>
          </div>
        )}

        {results && !results.error && (
          <div className="mt-8 bg-black border border-gray-800 rounded-xl p-4 md:p-6 font-mono relative group animate-in fade-in">
            
            <button 
              onClick={handleDownloadReport} 
              className="md:absolute static w-full md:w-auto mt-4 md:mt-0 top-4 right-4 bg-gray-800 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all opacity-100 md:opacity-80 md:group-hover:opacity-100 z-10 mb-4 md:mb-0"
            >
            ...
            {results.vulnerabilities ? (
               <div className="pt-2 md:pt-8">
                 <h3 className="text-emerald-400 text-sm md:text-lg mb-4 break-all">{'>'} Target: {results.target}</h3>
                 ...
                 {results.vulnerabilities.map((vuln, i) => (
                   <div key={i} className="mb-3 p-4 border border-red-900/50 bg-red-950/20 rounded-lg">
                     ...
                     <span className="text-gray-400 text-xs md:text-sm">Endpoint: {vuln.endpoint || 'N/A'}</span><br/>
                     <span className="text-gray-300 text-sm mt-2 block">{vuln.desc}</span>
                   </div>
                 ))}
               </div>
            ) : (
               <div className={`p-4 md:mt-8 rounded-lg border ${results.status === 'danger' ? 'border-red-500/50 bg-red-950/20' : 'border-emerald-500/50 bg-emerald-950/20'}`}>
                 <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4">
                   {results.status === 'danger' ? <AlertTriangle className="w-8 h-8 text-red-500 shrink-0" /> : <ShieldCheck className="w-8 h-8 text-emerald-500 shrink-0" />}
                   <div>
                     <h4 className={`text-lg md:text-xl font-bold ${results.status === 'danger' ? 'text-red-500' : 'text-emerald-500'}`}>{results.message}</h4>
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0a] text-gray-200 font-sans pb-20 md:pb-0">
      {renderSidebar()}
